...
{ esp_err_t ret;
  ...
  ret = i2c_master_write_byte(cmd, (FDC2214_I2C_ADDRESS<<1) | WRITE_BIT, ACK_CHECK_EN);
  if(ret!=ESP_OK)
  ...
  ret = i2c_master_write_byte(cmd, (FDC2214_I2C_ADDRESS<<1) | READ_BIT, ACK_CHECK_EN);
  if(ret!=ESP_OK)
...
void FDC2212::loadSettings(void) {
    ...
    uint16_t FDC2212_CONFIG_REGADDR_VAL = ACTIVE_CHAN | 
	                                      SLEEP_MODE_EN |
										  RESERVED_1 |
										  SENSOR_ACTIVATE_SEL |
										  RESERVED_2 |
...
//    write16FDC(FDC2214_CONFIG_REGADDR, 0x201);  //set config
	...
	Serial.printf("Conf register value: %d %x ", FDC2212_CONFIG_REGADDR_VAL,FDC2212_CONFIG_REGADDR_VAL);
    ...
    uint16_t FDC2212_ERROR_CONFIG_VAL = ERROR_CONFIG_RES1 |
	                                    WD_ERR2OUT |
										AH_WARN2OUT |
										AL_WARN2OUT |
										ERROR_CONFIG_RES2 |
	...
	Serial.printf("Error Conf register value: %d %x ", FDC2212_ERROR_CONFIG_VAL,FDC2212_ERROR_CONFIG_VAL);
	Serial.print(FDC2212_ERROR_CONFIG_VAL,BIN);
	Serial.println();	

	Serial.printf("Error Conf register value: %d %x ", errorConfVal,errorConfVal);
...
//        Serial.println("status: " + String(status));
		...
		if(capSlow==0)
           ...
           capSlow = slowFilter * (float)reading + (1-slowFilter) * capSlow;
		
		if(capFast==0)
		   capFast = (float)reading;
		else
		   capFast = fastFilter * (float)reading + (1-fastFilter) * capFast;
		}
/*
        Serial.println();
...
*/
        if(abs(capSlow - capFast) < triggerDiff && _response.status == true)
		{
