...
String scanNetworks(){
  ...
  if (n == 0) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        ret.concat("wifi ");
        ret.concat(WiFi.SSID());
        ret.concat(" (");
        ret.concat(WiFi.RSSI());
        ret.concat(") ");
...
/*
  ...
  */
  ...
  server.serveStatic("/", SPIFFS, "/").setCacheControl("max-age=600").setDefaultFile("index.html").setTemplateProcessor(processor);
