...
>>>>>>> branch 'master' of https://github.com/zhivko/DoubleLifter.git
...
//message is comprised of multiple frames or the frame is split into multiple packets
			...
			Serial.printf("%lu ws[%s][%u] frame[%u] %s[%llu - %llu]: ", millis(),
...
//@formatter:off
<<<<<<< HEAD
	snprintf(txtToSend, sizeof(txtToSend)-1,
			jsonTemplateStr.c_str(),
			encoder1_value,
			encoder2_value,
			pwm1,
			pwm2,
			...
			,size);
=======
	snprintf(txtToSend, sizeof(txtToSend) - 1, jsonTemplateStr,
			encoder1_value, encoder2_value, pwm1, pwm2, target1, target2, output1,
			output2, an1, an2, (pid1.getActual() - pid2.getActual()),
			pid1.getPoutput(), pid1.getIoutput(), pid1.getDoutput(),
			pid1.getPOSoutput(), pid1.getSetpoint(), pid1.getActual(),
			pid1.getError(), pid1.getErrorSum(), pid1.getMaxIOutput(),
			pid1.getMaxError(), pid2.getPoutput(), pid2.getIoutput(),
			pid2.getDoutput(), pid2.getPOSoutput(), pid2.getSetpoint(),
			pid2.getActual(), pid2.getError(), pid2.getErrorSum(),
			pid2.getMaxIOutput(), pid2.getMaxError(), (unsigned int)stop1_top, (unsigned int)stop1_bottom,
			(unsigned int)stop2_top, (unsigned int)stop2_bottom, cap_reading, fdc2212.readTimeMs, fdc2212.capFast,
			fdc2212.capSlow, (float)timeH, (pidEnabled == true ? "1" : "0"), ESP.getFreeHeap());
>>>>>>> branch 'master' of https://github.com/zhivko/DoubleLifter.git
//@formatter:on
}
